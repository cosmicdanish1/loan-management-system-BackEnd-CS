...
async function main() {
    ...
    for (const t of TARGETS) {
        ...
        const r = await svc.recordLoanRepayment({
            ...
            receiptNo: 'SEPCHECK2', narration: 'Through-September installments (test)', username: 'sep-check',
            asOfDate: CLOSE_DATE,
        });
    ...
    for (const r of rollbackInfo) {
        await AppDataSource.query(`DELETE FROM loan_repayment_ledger WHERE loancaseno::text = $1 AND receipt_no = 'SEPCHECK2'`, [r.case]);
